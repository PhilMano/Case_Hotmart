...
spark.sql("""
          select p.producer_id,   
            ...
            from  purchase_gold p
          inner join product_item_gold pi on pi.prod_item_id = p.prod_item_id and pi.prod_item_partition = p.prod_item_partition    
          where 1=1 
...
# MAGIC %md
# MAGIC Para aumento de performance tem-se o exemplo abaixo no qual os filtros foram aplicados no join em vez do where, nao nescessitando de uma isntrucao a mais que seria o where. Isso decorre do fato do join ser executado antes do where da arvore de execucao do SQL.

# COMMAND ----------

# MAGIC %md
# MAGIC ![](/Workspace/Users/philipemanoel70@gmail.com/Case_Hotmart/Exercicio_1/arvore_execucao_sql)
...
spark.sql("""
                with product_billing as (
                    ...
                    from  purchase_gold p
                    join product_item_gold pi on pi.prod_item_id = p.prod_item_id and pi.prod_item_partition = p.prod_item_partition
                    where 1=1
                          and p.release_date is not null
...
# MAGIC %md
# MAGIC Executando filtro no Join.
